...
if (isset($_POST['savedata'])) {
    ...
    try {
        ...
        if ($insert_query_run === true) {
            ...
            header('location: /benguetlivestock/frontend/livestock-volume-trend.php');
        } else {
    ...
    $update_query = "UPDATE livestockvolumetrend SET cattle_volume=?, swine_volume=?, carabao_volume=?, goat_volume=?, chicken_volume=?, duck_volume=?, fish_volume=?, date_updated=? WHERE year=?";
    $stmt = mysqli_prepare($connection, $update_query);
    mysqli_stmt_bind_param($stmt, "iiiiiiisi", $update_cattle, $update_swine, $update_carabao, $update_goat, $update_chicken, $update_duck, $update_fish, $update_date, $update_year);
    ...
    $delete_query = "DELETE FROM livestockvolumetrend WHERE year='$id'";
